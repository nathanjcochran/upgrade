...
the go.mod file (adding the version component if necessary). The same behavior
is triggered by supplying the module's own path for the [module] argument.
However, in that form, a target [version] can also be given, making it possible
to jump several major versions at once (or to downgrade versions).
...
well as the relevant import paths in all Go files in the current module (the
current package and all subpackages).

If the special target "all" is given, the tool will attempt to upgrade all
direct dependencies in the go.mod file to the highest major version available.

This tool does not add version tags in version control systems.
...
The [version], if given, must be a valid semver module version. It can be
provided with any level of major/minor/patch specificity - e.g. 'v2', 'v2.3',
'v.2.3.4'. When upgrading a dependency, the tool will attempt to upgrade to the
highest available matching version, unless the target major version of the
dependency is already required, in which case it will maintain the existing
minor/patch version. When upgrading the current module, only the major
component of the provided version is taken into account.
